...
/*
  ...
  and every lookup fell through to 'Unverified User'. The name now
  comes from data/kyc.json first (where it actually lives), falling
  back to users.json only for the separate, currently-unused
  register/login path, in case that's ever wired up later.
*/
function sellerPublicInfo(sellerId) {
  const kycRecord = read('kyc').find(
    k => String(k.userId) === String(sellerId)
  );

  const user = read('users').find(
    u => String(u.userId) === String(sellerId)
  );
  ...
  return {
    sellerName: fullName,
    sellerInitial: fullName.trim().charAt(0).toUpperCase() || '?',
    sellerRating: rating + '%',
...
function withSellerInfo(ad) {
  return Object.assign({}, ad, sellerPublicInfo(ad.sellerId));
}
