...
app.post(
  ...
  (req, res) => {
    ...
    const tierData = tierForUser(uid);
    const fee = Number(tierData.config.withdrawalFee || 0);
    ...
    if (
      ...
      return res.status(400).json({
        error:
          `Insufficient wallet balance. Withdrawal fee is ${fee.toFixed(2)} ETB.`
      });
    }
      ...
      if (
        ...
        const withdrawalFee = Number(
          transaction.fee !== undefined
            ? transaction.fee
            : tierForUser(transaction.userId).config.withdrawalFee || 0
        );
        const debit = Number(
          (Number(transaction.amount) + withdrawalFee).toFixed(2)
        );
        ...
        transaction.totalDebit = debit;
        if (withdrawalFee > 0) {
          addRevenue(
            'withdrawal_fee',
            withdrawalFee,
            { userId: transaction.userId, transactionType: 'withdrawal' }
          );
        }
      }
...
app.post(
  ...
  (req, res) => {
    ...
    const tierData = tierForUser(from);
    const fee = Number(tierData.config.transferFee || 0);
    const totalDebit = Number((amount + fee).toFixed(2));
    ...
    if (
      ...
      return res.status(400).json({
        error:
          `Insufficient balance. Transfer fee is ${fee.toFixed(2)} ETB.`
      });
    }
    ...
    if (fee > 0) addRevenue('transfer_fee', fee, { userId: from, transactionType: 'transfer' });
...
/* =========================================================
   PLATFORM CONFIG / TIERS / FEES / SUPPORT / CONTROLS
========================================================= */

// These features are deliberately stored in the same JSON datastore so the
// existing 3,269-line backend remains compatible with its current project.
Object.assign(files, {
  tiers: 'tiers.json',
...
function readObject(name, defaults) {
...
function defaultTiers() {
  return {
    '0': { dailyTransferLimit: 0, monthlyTransferLimit: 0, transferFee: 0, withdrawalFee: 0, platformFeeUSD: 0.38 },
    '1': { dailyTransferLimit: 50000, monthlyTransferLimit: 300000, transferFee: 0, withdrawalFee: 0, platformFeeUSD: 0.38 },
    '2': { dailyTransferLimit: 200000, monthlyTransferLimit: 1000000, transferFee: 0, withdrawalFee: 0, platformFeeUSD: 0.38 },
    '3': { dailyTransferLimit: 1000000, monthlyTransferLimit: 5000000, transferFee: 0, withdrawalFee: 0, platformFeeUSD: 0.38 }
  };
}
...
function getTiers() {
  ...
  const saved = readObject('tiers', defaults);
  for (const k of Object.keys(defaults)) {
    saved[k] = Object.assign({}, defaults[k], saved[k] || {});
  }
  return saved;
}

function getUserRecord(uid) {
  return read('users').find(u => String(u.userId) === String(uid));
}

function ensureUserControls(user) {
  if (!user) return null;
  let changed = false;
  if (typeof user.walletFrozen !== 'boolean') { user.walletFrozen = false; changed = true; }
  if (typeof user.suspended !== 'boolean') { user.suspended = false; changed = true; }
  if (!user.tier) { user.tier = '0'; changed = true; }
  if (changed) {
    const users = read('users');
    const i = users.findIndex(u => String(u.userId) === String(user.userId));
    if (i >= 0) { users[i] = user; write('users', users); }
  }
  return user;
}

function requireActiveUser(req, res, next) {
  const user = ensureUserControls(getUserRecord(userId(req)));
  if (!user) return res.status(401).json({ error: 'Please log in' });
  if (user.suspended) return res.status(403).json({ error: 'Account is suspended' });
  next();
}

function requireWalletActive(req, res, next) {
  const user = ensureUserControls(getUserRecord(userId(req)));
  if (!user) return res.status(401).json({ error: 'Please log in' });
  if (user.suspended) return res.status(403).json({ error: 'Account is suspended' });
  if (user.walletFrozen) return res.status(403).json({ error: 'Wallet is frozen' });
  next();
}

function tierForUser(uid) {
  const user = ensureUserControls(getUserRecord(uid));
  const tier = user && String(user.tier || '0');
  const tiers = getTiers();
  return { id: tiers[tier] ? tier : '0', config: tiers[tiers[tier] ? tier : '0'] };
}

function addRevenue(type, amountETB, meta = {}) {
  const data = readObject('revenue', { entries: [], ownerBalanceETB: 0 });
  const amount = Number(amountETB || 0);
  if (!Number.isFinite(amount) || amount <= 0) return;
  data.entries = Array.isArray(data.entries) ? data.entries : [];
  data.entries.push({ id: id('rev'), type, amount: Number(amount.toFixed(2)), createdAt: new Date().toISOString(), ...meta });
  data.ownerBalanceETB = Number((Number(data.ownerBalanceETB || 0) + amount).toFixed(2));
  write('revenue', data);
}
...
function getRevenueSummary() {
  ...
  const total = entries.reduce((n, x) => n + Number(x.amount || 0), 0);
  const todayTotal = entries.filter(x => String(x.createdAt || '').slice(0, 10) === today).reduce((n, x) => n + Number(x.amount || 0), 0);
  const monthTotal = entries.filter(x => String(x.createdAt || '').slice(0, 7) === month).reduce((n, x) => n + Number(x.amount || 0), 0);
  return {
    platformFeeUSD: 0.38,
    ownerWalletBalanceETB: Number(data.ownerBalanceETB || 0),
    totalFeesCollectedETB: Number(total.toFixed(2)),
    todayRevenueETB: Number(todayTotal.toFixed(2)),
    monthRevenueETB: Number(monthTotal.toFixed(2)),
    entries: entries.slice(-200).reverse()
  };
}

// Public read-only configuration needed by the user application.
app.get('/api/config', (req, res) => {
  const settings = getSettings();
  const tiers = getTiers();
  res.json({
    maintenanceMode: !!settings.maintenanceMode,
    exchangeRateUSDToETB: Number(settings.exchangeRateUSDToETB || 0),
...
app.get('/api/me/tier', requireUser, (req, res) => {
  const user = ensureUserControls(getUserRecord(userId(req)));
  const tier = tierForUser(userId(req));
  res.json({ tier: tier.id, settings: tier.config });
});

// Admin tier and fee manager.
app.get('/api/admin/tiers', requireAdmin, (req, res) => {
  res.json(getTiers());
});

app.post('/api/admin/tiers/:tier', requireAdmin, (req, res) => {
  const tierId = String(req.params.tier);
  if (!['0', '1', '2', '3'].includes(tierId)) return res.status(400).json({ error: 'Tier must be 0, 1, 2 or 3' });
  const fields = ['dailyTransferLimit', 'monthlyTransferLimit', 'transferFee', 'withdrawalFee', 'platformFeeUSD'];
  const tiers = getTiers();
  const next = { ...tiers[tierId] };
  for (const field of fields) {
    if (req.body[field] !== undefined) {
      const n = Number(req.body[field]);
      if (!Number.isFinite(n) || n < 0) return res.status(400).json({ error: `${field} must be a non-negative number` });
      next[field] = Number(n.toFixed(2));
    }
  }
  tiers[tierId] = next;
  write('tiers', tiers);
  res.json({ tier: tierId, settings: next });
});

app.post('/api/admin/users/:userId/tier', requireAdmin, (req, res) => {
  const tier = String(req.body.tier || '');
  if (!['0', '1', '2', '3'].includes(tier)) return res.status(400).json({ error: 'Tier must be 0, 1, 2 or 3' });
  const users = read('users');
  const user = users.find(u => String(u.userId) === String(req.params.userId));
...
// Admin user controls: freeze/unfreeze wallet and suspend/unsuspend account.
app.get('/api/admin/users', requireAdmin, (req, res) => {
  const q = String(req.query.q || '').trim().toLowerCase();
  let users = read('users').map(u => ensureUserControls(u)).map(u => ({
    ...safeUser(u), walletFrozen: !!u.walletFrozen, suspended: !!u.suspended, tier: u.tier || '0'
  }));
  if (q) users = users.filter(u => `${u.userId} ${u.username} ${u.fullName}`.toLowerCase().includes(q));
  res.json(users);
});

app.post('/api/admin/users/:userId/control', requireAdmin, (req, res) => {
  const users = read('users');
  const user = users.find(u => String(u.userId) === String(req.params.userId));
  if (!user) return res.status(404).json({ error: 'User not found' });
  if (req.body.walletFrozen !== undefined) user.walletFrozen = !!req.body.walletFrozen;
  if (req.body.suspended !== undefined) user.suspended = !!req.body.suspended;
  write('users', users);
  res.json({ user: safeUser(user), walletFrozen: !!user.walletFrozen, suspended: !!user.suspended, tier: user.tier || '0' });
});

// Settings / maintenance / broadcast control.
app.get('/api/admin/settings', requireAdmin, (req, res) => res.json(getSettings()));
app.post('/api/admin/settings', requireAdmin, (req, res) => {
  const current = getSettings();
  if (req.body.maintenanceMode !== undefined) current.maintenanceMode = !!req.body.maintenanceMode;
  if (req.body.exchangeRateUSDToETB !== undefined) {
    const n = Number(req.body.exchangeRateUSDToETB);
    if (!Number.isFinite(n) || n < 0) return res.status(400).json({ error: 'Invalid exchange rate' });
    current.exchangeRateUSDToETB = Number(n.toFixed(4));
  }
  if (req.body.broadcastMessage !== undefined) current.broadcastMessage = String(req.body.broadcastMessage || '').slice(0, 2000);
  current.updatedAt = new Date().toISOString();
  write('settings', current);
  if (current.broadcastMessage) {
    const users = read('users');
    for (const u of users) addNotification(u.userId, 'announcement', 'System announcement', current.broadcastMessage);
  }
  res.json(current);
});
...
app.get('/api/support/tickets', requireUser, (req, res) => {
  const uid = userId(req);
  const tickets = read('support').filter(t => String(t.userId) === String(uid)).reverse();
  res.json(tickets);
});
...
app.post('/api/support/tickets', requireUser, (req, res) => {
  ...
  const ticket = { id: id('ticket'), userId: userId(req), category, subject, status: 'open', messages: [{ id: id('sm'), from: 'user', body, createdAt: new Date().toISOString() }], createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
  const tickets = read('support'); tickets.push(ticket); write('support', tickets);
  res.status(201).json(ticket);
});

app.get('/api/admin/support/tickets', requireAdmin, (req, res) => res.json(read('support').reverse()));
...
app.post('/api/admin/support/tickets/:id/reply', requireAdmin, (req, res) => {
  ...
  const tickets = read('support');
  const ticket = tickets.find(t => t.id === req.params.id);
  if (!ticket) return res.status(404).json({ error: 'Ticket not found' });
  ticket.messages = Array.isArray(ticket.messages) ? ticket.messages : [];
  ticket.messages.push({ id: id('sm'), from: 'admin', body, createdAt: new Date().toISOString() });
  ticket.status = 'open'; ticket.updatedAt = new Date().toISOString();
  write('support', tickets);
  addNotification(ticket.userId, 'support', `Support reply: ${ticket.subject}`, body);
  res.json(ticket);
});

app.post('/api/admin/support/tickets/:id/close', requireAdmin, (req, res) => {
  const tickets = read('support');
  const ticket = tickets.find(t => t.id === req.params.id);
  if (!ticket) return res.status(404).json({ error: 'Ticket not found' });
  ticket.status = 'closed'; ticket.updatedAt = new Date().toISOString();
  write('support', tickets);
  addNotification(ticket.userId, 'support', 'Support ticket closed', `Your support ticket "${ticket.subject}" has been closed.`, '');
  res.json(ticket);
});

app.post('/api/support/tickets/:id/reply', requireUser, (req, res) => {
  const body = String(req.body.body || req.body.message || '').trim().slice(0, 2000);
  if (!body) return res.status(400).json({ error: 'Message is required' });
  const tickets = read('support');
  const ticket = tickets.find(t => t.id === req.params.id && String(t.userId) === String(userId(req)));
  if (!ticket) return res.status(404).json({ error: 'Ticket not found' });
  if (ticket.status === 'closed') return res.status(400).json({ error: 'Ticket is closed' });
  ticket.messages = Array.isArray(ticket.messages) ? ticket.messages : [];
  ticket.messages.push({ id: id('sm'), from: 'user', body, createdAt: new Date().toISOString() });
  ticket.status = 'open'; ticket.updatedAt = new Date().toISOString();
  write('support', tickets);
  res.json(ticket);
});
...
// Revenue dashboard.
app.get('/api/admin/revenue', requireAdmin, (req, res) => res.json(getRevenueSummary()));
