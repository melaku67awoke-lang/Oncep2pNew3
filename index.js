...
function sellerPublicInfo(sellerId) {
  ...
  const displayName = fullName || 'Registered User';
...
function withSellerInfo(ad) {
  ...
  // Prefer the immutable seller snapshot stored on the ad when available.
  // This keeps an existing ad's owner name stable even if profile/KYC data
  // is later migrated, while new ads still resolve from the exact owner.
  ...
  return Object.assign({}, ad, resolved, {
    sellerName: snapshotName || resolved.sellerName,
    sellerInitial: snapshotInitial || resolved.sellerInitial
  });
}
