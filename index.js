...
/*
  ...
  NOTE: posting/managing an ad only requires requireUser (a valid
  logged-in session) -- it intentionally does NOT also require
  requireVerified. KYC approval is a separate gate that already
  controls whether the app lets someone reach the main app / Post Ad
  screen at all (see checkKYCStatusOnStartup / applyKYCStatus), so
  re-checking it here just rejected already-verified users with a
  confusing "KYC approval required" error.
*/
