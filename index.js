...
/*
  How long a seller has to accept a newly-created order before it is
  auto-cancelled and the buyer's escrow is released back to them.
*/
const ORDER_ACCEPT_WINDOW_MS = 5 * 60 * 1000;

/*
  Returns the real, current deadline for an order given its status, or
  null if that status has no active countdown. Centralized here so
  GET /api/orders and autoExpireOrders() can never disagree about what
  "the deadline" means for a given status.
*/
function computeOrderDeadline(order) {
  if (order.status === 'pending_acceptance') {
    return order.acceptBy || null;
  }

  if (order.status === 'paid_escrow') {
    return order.shipBy || null;
  }

  if (['shipped', 'delivered'].includes(order.status)) {
    return order.completeBy || null;
  }
...
/*
  ...
  SELLER-ACCEPT TIMEOUT: a freshly created order now starts out as
  'pending_acceptance' with a 5-minute acceptBy deadline instead of
  jumping straight to 'paid_escrow'. If the seller doesn't call
  POST /api/orders/:id/accept before that deadline, this sweep treats it
  exactly like any other expiry: the buyer's held escrow is released
  back to their spendable balance, the order moves to 'cancelled', and
  the listing (if any) is reopened so it can be bought again.
*/
function autoExpireOrders() {
  ...
  orders.forEach(order => {
    if (
      !['pending_acceptance', 'paid_escrow', 'shipped', 'delivered'].includes(
        order.status
      )
    ) {
      return;
    ...
    const isAcceptTimeout = order.status === 'pending_acceptance';
    ...
    order.cancelReason = isAcceptTimeout ? 'seller_no_response' : 'expired';
    ...
    if (isAcceptTimeout) {
      addNotification(
        ...
        'order_update',
        'Order cancelled',
        'The seller did not respond in time, so the order was cancelled and your escrow was refunded.',
        order.id
      );
      ...
      addNotification(
        ...
        'order_update',
        'Order cancelled',
        'You did not accept the order in time, so it was automatically cancelled.',
        order.id
      );
    } else {
      addNotification(
...
/*
  Run the same sweep on a timer, independently of anyone hitting
  GET /api/orders -- so a seller's 5-minute accept window (and the
  other order deadlines) are enforced even if neither party has the
  app open when the deadline passes.
*/
setInterval(autoExpireOrders, 15 * 1000);
autoExpireOrders();
...
app.post(
  ...
  (req, res) => {
    ...
    const order = {
      ...
      /*
        SELLER-ACCEPT TIMEOUT: the order no longer jumps straight to
        'paid_escrow' on creation. The buyer's funds are moved into
        escrow immediately (below), but the order sits in
        'pending_acceptance' -- waiting on the seller to call
        POST /api/orders/:id/accept -- until either that happens or
        acceptBy passes, at which point autoExpireOrders() cancels it
        and refunds the buyer automatically.
      */
      ...
      acceptBy: new Date(
        Date.now() + ORDER_ACCEPT_WINDOW_MS
      ).toISOString(),

      // Set once the seller accepts (see /accept below); null until then.
      shipBy: null,
      completeBy: null,
      expiresAt: null,
    ...
    addNotification(
      listing.sellerId,
      'order_update',
      'New order awaiting your acceptance',
      `Someone wants to buy your listing "${listing.title}". Accept within 5 minutes or it will be automatically cancelled.`,
      order.id
    );
...
/* =========================================================
   ACCEPT ORDER (seller responds within the 5-minute window)
========================================================= */
...
app.post(
  ...
  (req, res) => {
    ...
    if (
      ...
      // The 5-minute window already passed -- let the regular expiry
      // sweep (which also refunds the buyer) take care of it instead
      // of accepting a stale order here.
      autoExpireOrders();

      return res.status(400).json({
        error: 'The acceptance window has expired'
      });
    }
    ...
    order.expiresAt = order.completeBy;
    ...
    addNotification(
      ...
      'order_update',
      'Order accepted',
      'The seller accepted your order. You can now proceed with the trade.',
      order.id
    );
...
/* =========================================================
   DECLINE ORDER (seller opts out before the 5-minute window ends)
========================================================= */
...
app.post(
  ...
  (req, res) => {
    ...
    if (String(order.sellerId) !== String(uid)) {
      return res.status(403).json({
        error: 'Only the seller can decline this order'
      });
    }

    if (order.status !== 'pending_acceptance') {
      return res.status(400).json({
        error: 'Order cannot be declined at this stage'
      });
    }
    ...
    if (
      ...
      return res.status(400).json({
        error: 'Escrow funds are unavailable'
      });
    }
    ...
    buyerWallet.heldBalance = Number(
      (Number(buyerWallet.heldBalance) - amount).toFixed(2)
    );

    buyerWallet.balance = Number(
      (Number(buyerWallet.balance || 0) + amount).toFixed(2)
    );

    order.status = 'cancelled';
    order.cancelledBy = uid;
    order.cancelReason = 'declined_by_seller';
    order.updatedAt = new Date().toISOString();
    ...
    addNotification(
      ...
      'order_update',
      'Order declined',
      'The seller declined the order and your escrow was refunded.',
      order.id
    );
