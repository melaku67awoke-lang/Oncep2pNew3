...
app.get(
  ...
  (req, res) => {
    ...
    /*
      BUG FIX -- P2P ADS: the marketplace list is public (no session
      required to browse it), but the client still needs to show who
      posted each ad. The real display name lives on the KYC record
      (this app's actual identity store -- see the `users` vs `kyc`
      note on GET /api/orders below), so look it up here instead of
      leaving the client to guess.
    */
    const kycRecords = read('kyc');
    listings = listings.map(listing => {
      const sellerKyc = kycRecords.find(
        k => String(k.userId) === String(listing.sellerId)
      );
      return Object.assign({}, listing, {
        sellerName: sellerKyc ? sellerKyc.fullName : 'Trader'
      });
    });
...
app.post(
  ...
  (req, res) => {
    ...
    /*
      BUG FIX -- P2P ADS: the Buy/Sell ad form on the client posts a
      few extra fields (the ETB rate, min/max ETB order limits and the
      accepted payment methods) that describe the ad but are not part
      of the generic marketplace-listing shape above. These used to be
      silently dropped, so ads created through the P2P flow never had
      anywhere on the server to live -- store them on the listing (as
      plain optional metadata; they don't affect escrow, which is
      still driven by `price`, the USDT amount held for this listing).
    */
    const rate = Number(req.body.rate);
    const minOrderETB = Number(req.body.minOrderETB);
    const maxOrderETB = Number(req.body.maxOrderETB);
    ...
    const listing = {
      ...
      minOrderETB: Number.isFinite(minOrderETB) ? minOrderETB : 0,
      maxOrderETB: Number.isFinite(maxOrderETB) ? maxOrderETB : 0,
      payment: payment.slice(0, 200),
      status: 'active',
      createdAt:
        new Date().toISOString()
    };
...
app.post(
  ...
  (req, res) => {
    ...
    const order = {
      ...
      /*
        BUG FIX -- ORDERS BUG: this is the P2P payment window the
        client renders as a live mm:ss countdown (see startOrderTimers
        / renderOngoingOrders in public/index.html, which starts every
        new order's timer at "05:00"). It was set to 48 hours, which
        both broke the mm:ss display and meant a buyer's escrow never
        got auto-refunded on any timescale that matched what the UI
        promised. Five minutes to submit payment, matching the UI.
      */
      shipBy: new Date(
        Date.now() +
          5 * 60 * 1000
...
app.get(
  ...
  (req, res) => {
    ...
    /*
      BUG FIX -- ORDERS BUG: this used to look counterparties up in the
      `users` collection, which only ever gets an entry from the old
      email/password /api/auth/register flow. The app doesn't actually
      use that flow -- every real identity here is a KYC-verified
      `kycUserId` (see requireVerified / /api/wallet/session), and
      those users are never added to `users`. So `users.some(...)`
      was false for every real buyer and seller, and this filter threw
      away every single order before it ever reached the client --
      "Order ongoing page is empty" and "order can't go to the other
      user" for every order, every time. Look counterparties up in
      `kyc` instead, which is this app's real, populated identity
      store.
    */
    const kycRecords = read('kyc');
    ...
    const orders = read('orders')
      ...
      .filter(order => {
        // If the other party's KYC record is genuinely gone (e.g. an
        // old sample account was deleted server-side), there's no one
        // left to trade with -- don't show it.
        const counterpartyId =
          String(order.buyerId) === String(uid)
            ? order.sellerId
            : order.buyerId;

        return kycRecords.some(
          k => String(k.userId) === String(counterpartyId)
        );
      })
      .map(order => {
        const isBuyer = String(order.buyerId) === String(uid);
        const counterpartyId = isBuyer ? order.sellerId : order.buyerId;
        const counterparty = kycRecords.find(
          k => String(k.userId) === String(counterpartyId)
        );
