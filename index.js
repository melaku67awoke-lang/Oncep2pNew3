...
// ===== V119 REAL KYC ADMIN FIX =====
app.get('/api/kyc/status/:userId', (req,res)=>{
  const kyc = read('kyc').find(k=>String(k.userId)===String(req.params.userId));
...
// Admin endpoint returns all KYC submissions
app.get('/api/admin/kyc/all', requireAdmin, (req,res)=>{
  const users = read('users');
  const list = read('kyc').map(k=>{
    const u = users.find(x=>String(x.userId)===String(k.userId)) || {};
    return {
      ...k,
      username: u.username || '',
