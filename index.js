...
// Production security middleware without requiring extra packages.
...
const ADMIN_KEY = process.env.ADMIN_KEY || (process.env.NODE_ENV === 'production' ? '' : 'change-me-admin-key');
...
function userId(req) {
  ...
  if (active.length !== sessions.length) write('sessions', active);
...
// KYC uploads are intentionally NOT exposed as a public static directory.
// The admin KYC endpoint below returns document data only after admin authentication.
...
app.post('/api/auth/login', rateLimit(5, 15 * 60 * 1000), (req, res) => {
  try {
    const username = String(
      req.body.username || ''
    ).trim();
  ...
  (req, res) => {
    ...
    const data = read('kyc')
      ...
      .map(k => {
        ...
        return {
          ...k,
          telegramId: u && (u.telegramId || u.telegram || u.telegramID) || '',
          idDocument: toDataUrl(k.idDocument),
          selfie: toDataUrl(k.selfie)
        };
      });
    res.json(data);
  }
...
/* =========================================================
   ADMIN CONTROL / TIERS / SUPPORT / SETTINGS / USERS / REVENUE
========================================================= */

const adminExtraFiles = {
  tiers: 'tiers.json',
  support: 'support.json',
  settings: 'settings.json'
};

for (const name of Object.values(adminExtraFiles)) {
  const p = path.join(DATA, name);
  if (!fs.existsSync(p)) {
    fs.writeFileSync(p, name === 'settings.json'
      ? JSON.stringify({ maintenance: false, exchangeRate: 0, broadcast: '' }, null, 2)
      : '[]', 'utf8');
  }
}

function readExtra(name) {
  const p = path.join(DATA, adminExtraFiles[name]);
  try { return JSON.parse(fs.readFileSync(p, 'utf8')); }
  catch { return name === 'settings' ? { maintenance: false, exchangeRate: 0, broadcast: '' } : []; }
}
function writeExtra(name, value) {
  const p = path.join(DATA, adminExtraFiles[name]);
  const tmp = p + '.tmp';
...
app.get('/api/admin/tiers', requireAdmin, (req, res) => {
  const defaults = [0,1,2,3].map(tier => ({ tier, dailyLimit: 0, monthlyLimit: 0, transferFee: 0, withdrawalFee: 0, platformFeeUsd: 0.38 }));
  const saved = readExtra('tiers');
  res.json(defaults.map(d => Object.assign(d, saved.find(x => Number(x.tier) === d.tier) || {})));
});
...
app.put('/api/admin/tiers/:tier', requireAdmin, (req, res) => {
  ...
  const n = v => Number.isFinite(Number(v)) && Number(v) >= 0 ? Number(Number(v).toFixed(2)) : 0;
  const all = readExtra('tiers').filter(x => Number(x.tier) !== tier);
  const item = { tier, dailyLimit:n(req.body.dailyLimit), monthlyLimit:n(req.body.monthlyLimit), transferFee:n(req.body.transferFee), withdrawalFee:n(req.body.withdrawalFee), platformFeeUsd:n(req.body.platformFeeUsd) };
  all.push(item); writeExtra('tiers', all); res.json(item);
});

app.get('/api/admin/support', requireAdmin, (req, res) => res.json(readExtra('support').reverse()));
app.post('/api/admin/support', requireAdmin, (req, res) => {
  const ticket = { id:id('sup'), userId:String(req.body.userId || ''), subject:String(req.body.subject || 'Support').slice(0,120), message:String(req.body.message || '').slice(0,2000), status:'open', replies:[], createdAt:new Date().toISOString(), updatedAt:new Date().toISOString() };
  if (!ticket.userId || !ticket.message) return res.status(400).json({ error:'User ID and message are required' });
  const all=readExtra('support'); all.push(ticket); writeExtra('support',all); res.status(201).json(ticket);
});
app.post('/api/admin/support/:id/reply', requireAdmin, (req,res) => {
  const all=readExtra('support'); const t=all.find(x=>x.id===req.params.id); if(!t) return res.status(404).json({error:'Ticket not found'});
  const body=String(req.body.message || '').trim().slice(0,2000); if(!body) return res.status(400).json({error:'Reply is required'});
  t.replies=t.replies||[]; t.replies.push({id:id('rep'), from:'admin', body, createdAt:new Date().toISOString()}); t.updatedAt=new Date().toISOString();
  writeExtra('support',all); addNotification(t.userId,'support','Support reply',body); res.json(t);
});
app.post('/api/admin/support/:id/close', requireAdmin, (req,res) => {
  const all=readExtra('support'); const t=all.find(x=>x.id===req.params.id); if(!t) return res.status(404).json({error:'Ticket not found'});
  t.status='closed'; t.updatedAt=new Date().toISOString(); writeExtra('support',all); res.json(t);
});

app.get('/api/admin/settings', requireAdmin, (req,res) => res.json(readExtra('settings')));
app.put('/api/admin/settings', requireAdmin, (req,res) => {
  const old=readExtra('settings'); const settings={ maintenance:!!req.body.maintenance, exchangeRate:Number(req.body.exchangeRate)||0, broadcast:String(req.body.broadcast||'').slice(0,2000) };
  writeExtra('settings',settings);
  if (settings.broadcast && settings.broadcast !== old.broadcast) {
    for (const u of read('users')) addNotification(u.userId,'announcement','Announcement',settings.broadcast);
  }
  res.json(settings);
});

app.get('/api/admin/revenue', requireAdmin, (req,res) => {
  const tx=read('transactions'); const fees=tx.filter(x=>String(x.type||'').toLowerCase().includes('fee'));
  ...
  res.json({ platformFeeUsd:0.38, ownerWalletBalance:0, totalFeesCollected:Number(sum(fees).toFixed(2)), today:Number(sum(fees.filter(x=>String(x.createdAt||'').startsWith(day))).toFixed(2)), month:Number(sum(fees.filter(x=>String(x.createdAt||'').startsWith(month))).toFixed(2)) });
});

app.get('/api/admin/users', requireAdmin, (req,res) => {
  const q=String(req.query.q||'').trim().toLowerCase();
  let users=read('users'); if(q) users=users.filter(u=>[u.userId,u.username,u.fullName,u.telegramUsername].some(v=>String(v||'').toLowerCase().includes(q)));
  res.json(users.map(u=>({userId:u.userId,username:u.username,fullName:u.fullName,telegramUsername:u.telegramUsername||'',frozen:!!u.frozen,suspended:!!u.suspended,createdAt:u.createdAt})));
});
app.post('/api/admin/users/:id/control', requireAdmin, (req,res) => {
  const users=read('users'); const u=users.find(x=>String(x.userId)===String(req.params.id)); if(!u) return res.status(404).json({error:'User not found'});
  const action=String(req.body.action||''); if(!['freeze','unfreeze','suspend','unsuspend'].includes(action)) return res.status(400).json({error:'Invalid action'});
  if(action==='freeze') u.frozen=true; if(action==='unfreeze') u.frozen=false; if(action==='suspend') u.suspended=true; if(action==='unsuspend') u.suspended=false;
  write('users',users); addNotification(u.userId,'account','Account update',`Your account status was updated: ${action}.`); res.json({userId:u.userId,frozen:!!u.frozen,suspended:!!u.suspended});
});
